...
const BuiltinPresets = {
  ...
  'Seq Demo': {
    modules: [
      { type: 'Sequencer', x: 140, y: 260, state: { bpm: 110, steps: 8, gateLen: 0.5, running: true, pattern: [
  ...
  'Seq Bassline': {
    modules: [
      { type: 'Sequencer', x: 140, y: 260, state: { bpm: 120, steps: 8, gateLen: 0.5, running: true, pattern: [
  ...
  'Seq Arp Minor': {
    modules: [
      { type: 'Sequencer', x: 140, y: 260, state: { bpm: 110, steps: 8, gateLen: 0.45, running: true, pattern: [
  ...
  'Seq Techno 16th': {
    modules: [
      { type: 'Sequencer', x: 140, y: 260, state: { bpm: 135, steps: 8, gateLen: 0.3, running: true, pattern: [
  ...
  'Seq Staccato': {
    modules: [
      { type: 'Sequencer', x: 140, y: 260, state: { bpm: 120, steps: 8, gateLen: 0.2, running: true, pattern: [
  ...
  'Seq Octaves': {
    modules: [
      { type: 'Sequencer', x: 140, y: 260, state: { bpm: 100, steps: 8, gateLen: 0.5, running: true, pattern: [
  ...
  'ADSR Sequence Pad': {
    modules: [
      ...
      { type: 'Sequencer', x: 130, y: 260, state: { steps: 8, gateLen: 0.6, pattern: [
  ...
  'ADSR Volume Lead': {
    modules: [
      ...
      { type: 'Sequencer', x: 130, y: 260, state: { steps: 8, gateLen: 0.5, pattern: [
...
function exportPatch() {
  // serialize modules with type, approximate position, and simple state via per-module toJSON
  ...
  // serialize connections by module title and port names (resolve by order/name)
  const findByType = (type) => list.find(l => l.type === type);
  const conns = [...connections.values()].map(c => ({
    from: [ modules.get(c.fromModuleId).constructor.name.replace('Module',''), c.fromPortName ],
    to: [ modules.get(c.toModuleId).constructor.name.replace('Module',''), c.toPortName ],
  }));
  return { modules: list.map(({id, ...rest}) => rest), connections: conns };
}
...
function loadPreset(preset) {
  ...
  preset.modules.forEach(m => {
    const inst = createModule(m.type, { x: m.x, y: m.y });
    // load simple state via fromJSON if available
    inst.fromJSON?.(m.state || {});
    if (!created[m.type]) created[m.type] = [];
    created[m.type].push(inst);
  });
  // connect
  preset.connections.forEach(c => {
    const [fromType, fromPort] = c.from;
    const [toType, toPort] = c.to;
    const from = Array.isArray(created[fromType]) ? created[fromType][0] : created[fromType];
    const to = Array.isArray(created[toType]) ? created[toType][0] : created[toType];
    if (!from || !to) return;
    const fromEl = from.getPortEl('out', fromPort);
    const toEl = to.getPortEl('in', toPort);
    if (!fromEl || !toEl) return;
    linking = { fromModuleId: from.id, fromPortEl: fromEl, fromPortName: fromPort, previewPath: createCablePath(true) };
    completeLink(to.id, toEl, toPort);
  });
}
